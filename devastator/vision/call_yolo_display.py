...
def main():
    ...
    while True:
        ...
        for obj in objects:
            ...
            cv2.putText(frame,
                        "#" + det_label + ' ' + str(round(obj['confidence'] * 100, 1)) + ' %' + ' ' + str(d[int(coords[1])][int(coords[0])]/1000) + " m",
                        (obj['xmin'], obj['ymin'] - 7), cv2.FONT_HERSHEY_COMPLEX, 0.6, color, 1)
            cv2.circle(frame, ( int(coords[0]), int(coords[1])), 5, (0,255,0), -1)
            print(det_label + ' ' + str(round(obj['confidence'] * 100, 1)) + ' %', str(d[int(coords[1])][int(coords[0])]/1000) + " m")
